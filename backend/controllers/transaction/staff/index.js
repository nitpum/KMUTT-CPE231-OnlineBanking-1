...
router.post('/deposit', async (req, res) => {
  try {
    ...
    const accountId = req.body.accountId
    const staffId = req.body.staffId
    const acc = await accountModel.account.transaction.deposit(accountId, amount)
    const trasactionAdd = await transactionModel.staff.deposit(amount, accountId, staffId)
    const result = {
      ...trasactionAdd, ...acc
    }
    res.send(result)
